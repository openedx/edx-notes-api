...
cd ..
git clone https://edx-deployment:${GH_ACCESS_TOKEN}@github.com/edx/edx-internal
...
../hub-linux*/bin/hub pull-request -m "${REPO_NAME} stage deploy: $GITHUB_UPSTREAM_PR_TITLE" -m "Staging environment deployment of https://github.com/edx/${REPO_NAME}/pull/$GITHUB_UPSTREAM_PR_NUMBER" -m "Review this PR to have Github deploy your code to stage.edx.org" -l staging-deployment -l automerge
...
sed -i -e "s/tag: .*/tag: $GITHUB_SHA-newrelic/" argocd/applications/${REPO_NAME}/edge-currently-deployed-image.yaml
git commit -a -m "${REPO_NAME} edge deploy: $GITHUB_UPSTREAM_PR_TITLE" --author "GitHub Actions CI Deployment automation <admin@edx.org>"
git push --set-upstream origin edx-deployment/edge/$GITHUB_SHA
../hub-linux*/bin/hub pull-request -m "${REPO_NAME} edge deploy: $GITHUB_UPSTREAM_PR_TITLE" -m "Edge environment deployment of https://github.com/edx/${REPO_NAME}/pull/$GITHUB_UPSTREAM_PR_NUMBER" -m "Review and merge this PR to deploy your code to edge.edx.org"
